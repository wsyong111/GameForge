pluginManagement {
	plugins {
		kotlin("jvm") version "2.0.0" apply false

		id("com.gradleup.shadow") version "9.0.0-beta15" apply false
		id("me.champeau.jmh") version "0.7.3" apply false
	}

	repositories {
		gradlePluginPortal()
	}
}

rootProject.name = "GameForge"

// FIXME: 给每个模块设定name
fun include(path: String, name: String? = null) {
	settings.include(path)

	if (name != null)
		project(path).name = name
}

include(":Assets")
include(":BuildInfo")
include(":Util")

include(":Framework")
include(":Framework:Annotation")
include(":Framework:App")
include(":Framework:Common")
include(":Framework:Config")
include(":Framework:Config:AnnotationProcessor")
include(":Framework:Context")
include(":Framework:DataFlow")
include(":Framework:Debug")
include(":Framework:EnvConfig")
include(":Framework:Event")
include(":Framework:I18n")
include(":Framework:Lang")
include(":Framework:Lifecycle")
include(":Framework:Listener")
include(":Framework:Mime")
include(":Framework:RichText")
include(":Framework:SPI")
include(":Framework:Tick")
include(":Framework:System:Audio")
include(":Framework:System:CrashReport")
include(":Framework:System:Graphic")
include(":Framework:System:Graphic:Core", "GraphicCore")
include(":Framework:System:Graphic:Render")
include(":Framework:System:Graphic:Surface")
include(":Framework:System:Graphic:VulkanBacked")
include(":Framework:System:Input")
include(":Framework:System:Log")
include(":Framework:System:Resource")
include(":Framework:System:Security")

include(":Game")
include(":Game:Core")
include(":Game:Core:ClientCore")
include(":Game:Core:CommonCore")
include(":Game:Core:ServerCore")
include(":Game:Client")
include(":Game:Common")
include(":Game:Server")
include(":Game:Loader")
include(":Game:Mod")
//include(":Game:Mod:Loader")
//include(":Game:Mod:Management")
//include(":Game:Mod:Runtime")
